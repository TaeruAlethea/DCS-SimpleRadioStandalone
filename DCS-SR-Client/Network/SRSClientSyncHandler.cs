...
public class SRSClientSyncHandler
{
	...
	public SRSClientSyncHandler(string guid, UpdateUICallback uiCallback, DCSRadioSyncHandler.NewAircraft _newAircraft)
	{
		...
		_idleTimeout = new DispatcherTimer(DispatcherPriority.Background, Application.Current.Dispatcher)
	...
	{
		if (_clientStateSingleton.ExternalAWACSModelSelected) return;
	...
	{
		if (!_clientStateSingleton.ExternalAWACSModelSelected || _radioDCSSync == null) return;
	...
	private void CallOnMain(bool result, bool connectionError = false)
	{
		...
		{
			Logger.Error(ex,
				"Failed to update UI after connection callback (result {result}, connectionError {connectionError})",
						...
						if (serverMessage != null)
							...
							switch (serverMessage.MsgType)
							{
								...
								case NetworkMessage.MessageType.CLIENT_DISCONNECT:
									...
									if (outClient != null) MessageHub.Instance.Publish(outClient);
								...
								case NetworkMessage.MessageType.EXTERNAL_AWACS_MODE_PASSWORD:
									...
									{
										Logger.Info("External AWACS mode authentication succeeded, coalition {0}",
	...
	public void Disconnect()
	{
		...
		try
		{
			if (_tcpClient != null) _tcpClient.Close(); // this'll stop the socket blocking
		}
		catch (Exception)
		{
		}
