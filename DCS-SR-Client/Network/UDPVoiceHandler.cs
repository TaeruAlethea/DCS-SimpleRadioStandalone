...
internal class UdpVoiceHandler
{
	...
	private readonly BlockingCollection<byte[]> _encodedAudio = new();
	...
	private List<RadioInformation> PTTPressed(out int sendingOn, bool voice)
	{
		...
		if (_clientStateSingleton.InhibitTX.InhibitTX)
		{
			...
			if (time.TotalSeconds < 5) return new List<RadioInformation>();
		}
				...
				if (transmittingRadios.Count > 0)
				{
					...
					if (currentlySelectedRadio != null &&
						...
						_audioManager.PlaySoundEffectStartTransmit(sendingOn,
							...
							VolumeConversionHelper.ConvertRadioVolumeSlider(currentlySelectedRadio.volume),
							currentlySelectedRadio.modulation);
	...
	private void StartPing()
	{
		...
		var thread = new Thread(() =>
		{
			...
			while (!_stop)
			{
				...
				try
				{
					if (_listener != null) _listener.Send(message, message.Length, _serverEndpoint);
				}
				catch (Exception e)
				{
