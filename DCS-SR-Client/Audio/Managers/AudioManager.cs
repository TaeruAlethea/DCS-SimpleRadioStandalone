...
public class AudioManager
{
	...
	private readonly byte[] _pcmBytes = new byte[MIC_SEGMENT_FRAMES * 2];

	//buffers intialised once for use repeatedly
	private readonly short[] _pcmShort = new short[MIC_SEGMENT_FRAMES];
	...
	private readonly byte[] tempBuffferFirst20ms = new byte[MIC_SEGMENT_FRAMES];
	private readonly byte[] tempBuffferSecond20ms = new byte[MIC_SEGMENT_FRAMES];
		...
		IPAddress ipAddress, int port)
	{
		...
		try
		{
			...
			_waveOut = speakers != null
				? new WasapiOut(speakers, AudioClientShareMode.Shared, true, 40)
				: new WasapiOut(AudioClientShareMode.Shared, 40);
		...
		if (micOutput != null) // && micOutput !=speakers
			try
			{
				...
				_micWaveOutBuffer =
					new BufferedWaveProvider(WaveFormat.CreateIeeeFloatWaveFormat(OUTPUT_SAMPLE_RATE, 1));
	...
	private void WasapiCaptureOnDataAvailable(object sender, WaveInEventArgs e)
	{
		...
		if (e.BytesRecorded > 0)
		{
			...
			{
				for (var i = 0; i < MIC_SEGMENT_FRAMES; i++) _pcmShort[i] = _micInputQueue.Dequeue();
				...
				try
				{
					...
					{
						Logger.Error(
							$"Invalid Bytes for Encoding - {_pcmShort.Length} should be {MIC_SEGMENT_FRAMES} ");
					}
