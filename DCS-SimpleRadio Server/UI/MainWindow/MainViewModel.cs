...
public sealed class MainViewModel : Screen, IHandle<ServerStateMessage>
{
	...
	private string _externalAWACSModeBluePassword =
		ServerSettingsStore.Instance.GetExternalAWACSModeSetting(ServerSettingsKeys.EXTERNAL_AWACS_MODE_BLUE_PASSWORD)
			.StringValue;

	private string _externalAWACSModeRedPassword =
		ServerSettingsStore.Instance.GetExternalAWACSModeSetting(ServerSettingsKeys.EXTERNAL_AWACS_MODE_RED_PASSWORD)
			.StringValue;
		...
		ClientAdminViewModel clientAdminViewModel)
	{
		...
		DisplayName = $"{Resources.TitleServer} - {UpdaterChecker.VERSION} - {ListeningPort}";
	...
	public string SpectatorAudioText
		=>
			ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.SPECTATORS_AUDIO_DISABLED).BoolValue
				? $"{Resources.BtnDisabled}"
				: $"{Resources.BtnEnabled}";
	...
	public string LOSText
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.LOS_ENABLED).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	public string RadioExpansion
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.RADIO_EXPANSION).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	public string AllowRadioEncryption
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.ALLOW_RADIO_ENCRYPTION).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";

	public string StrictRadioEncryption
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.STRICT_RADIO_ENCRYPTION).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	{
		get => _externalAWACSModeBluePassword;
		set
		{
	...
	{
		get => _externalAWACSModeRedPassword;
		set
		{
	...
	public string OverrideEffectsOnGlobal
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.RADIO_EFFECT_OVERRIDE).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	public string LotATCExportText
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.LOTATC_EXPORT_ENABLED).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	public string TransmissionLogEnabledText
		=> ServerSettingsStore.Instance.GetGeneralSetting(ServerSettingsKeys.TRANSMISSION_LOG_ENABLED).BoolValue
			? $"{Resources.BtnOn}"
			: $"{Resources.BtnOff}";
	...
	{
		var newSetting = RadioSecurityText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = SpectatorAudioText != $"{Resources.BtnDisabled}";
	...
	{
		var newSetting = ExportListText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = LOSText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = DistanceLimitText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = RealRadioText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = IRLRadioRxText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = RadioExpansion != $"{Resources.BtnOn}";
	...
	{
		var newSetting = AllowRadioEncryption != $"{Resources.BtnOn}";
	...
	{
		var newSetting = StrictRadioEncryption != $"{Resources.BtnOn}";
	...
	{
		var newSetting = CheckForBetaUpdates != $"{Resources.BtnOn}";
	...
	{
		var newSetting = OverrideEffectsOnGlobal != $"{Resources.BtnOn}";
	...
	{
		var newSetting = ExternalAWACSMode != $"{Resources.BtnOn}";
	...
	{
		ServerSettingsStore.Instance.SetExternalAWACSModeSetting(ServerSettingsKeys.EXTERNAL_AWACS_MODE_BLUE_PASSWORD,
			_externalAWACSModeBluePassword);
		ServerSettingsStore.Instance.SetExternalAWACSModeSetting(ServerSettingsKeys.EXTERNAL_AWACS_MODE_RED_PASSWORD,
			_externalAWACSModeRedPassword);
	...
	{
		ServerSettingsStore.Instance.SetGeneralSetting(ServerSettingsKeys.GLOBAL_LOBBY_FREQUENCIES,
			_globalLobbyFrequencies);
	...
	{
		var newSetting = TunedCountText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = LotATCExportText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = ShowTransmitterNameText != $"{Resources.BtnOn}";
	...
	{
		var newSetting = TransmissionLogEnabledText != $"{Resources.BtnOn}";
