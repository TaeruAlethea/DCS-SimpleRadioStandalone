...
public partial class MainWindow
{
	...
	public MainWindow()
	{
		...
		if (srPathStr != "") srPath.Text = srPathStr;
		...
		if (currentPath.StartsWith("file:\\")) currentPath = currentPath.Replace("file:\\", "");
	...
	private void SetupLogging()
	{
		...
		var logFilePath = Path.Combine(baseDirectory, "installer-log.txt");
		var oldLogFilePath = Path.Combine(baseDirectory, "install-log.old.txt");
	...
	private async void InstallReleaseButton(object sender, RoutedEventArgs e)
	{
		...
		new Action(async () =>
		{
			var result = await Task.Run(() => InstallRelease(srsPath, dcScriptsPath, shortcut));
			if (result == 0)
			{
				Application.Current.Dispatcher.Invoke(() =>
					{
	...
	private int InstallRelease(string srPath, string dcsScriptsPath, bool shortcut)
	{
		try
		{
			...
			if (dcsScriptsPath != null)
			{
				...
				foreach (var path in paths) InstallScripts(path);
			}
			else
			{
			...
			if (shortcut) InstallShortcuts(srPath);

			InstallVCRedist();

			if (dcsScriptsPath != null)
			{
				var message = Properties.Resources.MsgBoxInstallSuccessText;

				foreach (var path in paths) message += "\n" + path;
			...
			else
			{
				...
				{
					var message = Properties.Resources.MsgBoxInstallSuccessText2;
	...
	private static void ListFiles(string sDir)
	{
		try
		{
			...
			{
				foreach (var f in Directory.GetFiles(d)) Logger.Info(f);
				ListFiles(d);
			}
		}
		catch (Exception ex)
	...
	private static List<string> FindValidDCSFolders(string path)
	{
		...
		if (path == null || path.Length == 0) return paths;
	...
	private void InstallShortcuts(string path)
	{
		...
		var executablePath = Path.Combine(path, "SR-ClientRadio.exe");
	...
	{
		if (Directory.Exists(target_dir)) Directory.Delete(target_dir, true);
	}
	...
	private void DirectoryCopy(string sourceDirName, string destDirName)
	{
		...
		if (!Directory.Exists(destDirName)) CreateDirectory(destDirName);
